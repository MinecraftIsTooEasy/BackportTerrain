...
public class BWorldGenBigTree extends BWorldGenAbstractTree {
	...
	public BWorldGenBigTree(boolean notify) {
		super(notify);
	}
	...
	void func_150529_a(int x, int y, int z, float radius, byte major, Block block) {
		int l = (int) ((double) radius + 0.618D);
		byte b1 = otherCoordPairs[major];
		byte b2 = otherCoordPairs[major + 3];
		int[] aint = new int[] {x, y, z};
		int[] aint1 = new int[] {0, 0, 0};
		int i1 = -l;
		int j1 = -l;

		for (aint1[major] = aint[major]; i1 <= l; ++i1) {
			aint1[b1] = aint[b1] + i1;
			j1 = -l;
			...
			while (j1 <= l) {
				...
				if (d0 > (double) (radius * radius)) {
					++j1;
				} else {
					aint1[b2] = aint[b2] + j1;
					int id = this.worldObj.getBlockId(aint1[0], aint1[1], aint1[2]);

					if (id != 0 && id != Block.leaves.blockID) {
						++j1;
					} else {
						this.setBlockAndMetadata(this.worldObj, aint1[0], aint1[1], aint1[2], block.blockID, 0);
						++j1;
					}
				}
			}
	...
	void generateLeafNode(int x, int y, int z) {
		...
		for (int i1 = y + this.leafDistanceLimit; l < i1; ++l) {
			...
			this.func_150529_a(x, l, z, f, (byte) 1, Block.leaves);
		}
	}

	void func_150530_a(int[] from, int[] to, Block block) {
		...
		if (aint2[b1] != 0) {
			...
			for (int j = aint2[b1] + b4; i != j; i += b4) {
				aint3[b1] = MathHelper.floor_double((double) (from[b1] + i) + 0.5D);
				aint3[b2] = MathHelper.floor_double((double) from[b2] + (double) i * d0 + 0.5D);
				aint3[b3] = MathHelper.floor_double((double) from[b3] + (double) i * d1 + 0.5D);
				byte b5 = 0;
				int k = Math.abs(aint3[0] - from[0]);
				int l = Math.abs(aint3[2] - from[2]);
				int i1 = Math.max(k, l);
				...
				this.setBlockAndMetadata(this.worldObj, aint3[0], aint3[1], aint3[2], block.blockID, b5);
			}
		}
	}
	...
	void generateTrunk() {
		...
		this.func_150530_a(aint, aint1, Block.wood);
	...
	void generateLeafNodeBases() {
		...
		for (int[] aint = new int[] {this.basePos[0], this.basePos[1], this.basePos[2]}; i < j; ++i) {
			...
			if (this.leafNodeNeedsBase(k)) {
				this.func_150530_a(aint, aint2, Block.wood);
			}
		}
	}
	...
	int checkBlockLine(int[] from, int[] to) {
		...
		if (aint2[b1] == 0) {
			...
			for (j = aint2[b1] + b4; i != j; i += b4) {
				aint3[b1] = from[b1] + i;
				aint3[b2] = MathHelper.floor_double((double) from[b2] + (double) i * d0);
				aint3[b3] = MathHelper.floor_double((double) from[b3] + (double) i * d1);
	...
	boolean validTreeLocation() {
		...
		int soilId = this.worldObj.getBlockId(this.basePos[0], this.basePos[1] - 1, this.basePos[2]);
	...
	public void setScale(double heightLimitLimit, double scaleWidth, double leafDensity) {
		this.heightLimitLimit = (int) (heightLimitLimit * 12.0D);

		if (heightLimitLimit > 0.5D) {
